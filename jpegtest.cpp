...
void *rand_img_func(void *_done)
{
    ...
    for (int i = 0; i < _y; i++)
    {
        for (int j = 0; j < _x; j++)
        {
            for (int k = 0; k < 4; k++)
                // if (i % 2 && j % 2)
                imgval[i * _x * 4 + j * 4 + k] = 1; // build up image
            // printf("%02x ", img[i * j]);
        }
        // printf("\n");
    }
...
void ImageWindow(bool *active)
{
    ...
    if (mmy_image_texture != NULL)
    {
        ...
        ImGui::Text("size = %d x %d", mmy_image_width, mmy_image_height);
        ImGui::Image((void *)(intptr_t)mmy_image_texture, ImVec2(mmy_image_width, mmy_image_height));
    }
    ImGui::End();
}
...
int main(int argc, char *argv[])
{
    ...
    if (!glfwInit())
        return 1;
    GLFWwindow *window = glfwCreateWindow(1024, 720, "Camera Monitor", NULL, NULL);
    if (window == NULL)
        return 1;
    ...
    long sz = ftell(fp);
    fseek(fp, 0L, SEEK_SET);
    unsigned char jpegData[sz];
    fread(jpegData, 1, sz, fp);
    LoadTextureFromMem(jpegData, sz, &mmy_image_texture, &mmy_image_width, &mmy_image_height);
    fprintf(stderr, "%s: %d\n", __func__, __LINE__);
